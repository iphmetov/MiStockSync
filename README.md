...
- **Витя**: Только товары `"Имеются в нал."` и `"Распродажа"`
...
# Обновление цен
MIN_PRICE_CHANGE_PERCENT = 0.1     # Минимальное изменение для обновления (0.1%)
MAX_PRICE_CHANGE_PERCENT = 100.0   # Максимальное разрешенное изменение (100%)
SIGNIFICANT_CHANGE_PERCENT = 20.0  # Порог "значительного" изменения (20%)
```
...
- **Минимальное изменение**: < 0.1% - пропускается
- **Значительное изменение**: > 20% - попадает в отчет "Предупреждения"
- **Товары с ценой 0 в базе**: обновляются всегда
